...
1390 rem PRINT"{down*2} Source-Filename{space*6}: ";
1400 rem GOSUB 1040
1410 rem SN$=A$
1420 rem PRINT"{down} Destination-Filename : ";
1430 rem GOSUB 1040
1440 rem DN$=A$
1450 PRINT"Format loops (y/n)";
1460 POKE204,0
1470 GETFO$
1480 IFFO$<>"j"ANDFO$<>"y"ANDFO$<>"n"THEN 1470
1490 POKE204,1
1500 PRINTFO$
1510 rem PRINT"{down} Startadresse{space*2}: 2049{left*4}";
1520 rem GOSUB 1040
1530 SA=2049
1540 rem PRINT"{down} Startzeilennr.: 1000{left*4}";
1550 rem GOSUB 1040
1560 SZ=100
1570 rem PRINT"{down} Schrittweite{space*2}: 10{left*2}";
1580 rem GOSUB 1040
1590 SW=10
1600 PRINT"{clear}{down}{space*6}Recompactor{space*4}by GD-Soft {ct h}{ct n}"
1610 rem PRINT"{down*2}{space*3}Sourcefile{space*6}: "SN$
1620 rem PRINT"{down}{space*3}Destinationfile : "DN$
1630 :
1640 REM*****        PASS 1        *****
1650 :
1660 PRINT"{home}{down*10}"TAB(3)"Pass 1 :"
1670 rem OPEN1,GA,15
1680 rem OPEN2,GA,0,SN$
1690 rem GOSUB 1090
1700 rem IFDTHENCLOSE2:CLOSE1:GOSUB 3600:RUN
1710 GOSUB 1130
1720 GOSUB 1130
1730 IFAD=0THEN 1840
1740 GOSUB 1130
...
3310 rem OPEN1,GA,15
3320 rem OPEN2,GA,1,DN$
3330 rem GOSUB 1090
3340 rem IFDTHENCLOSE2:CLOSE1:GOSUB 3600:GOTO 3300
3350 AD=SA
3360 LB=255ANDSA
3370 HB=SA/256
...
3580 D$=ERR$
3590 D=ERR
3600 PRINT"{home}{down*21}{right}error :"
3610 PRINT"{down}{sh space}"D$;
3620 IFD<>VAL(D$)THENPRINT"{right}in";D;
3630 rem FORT=1TO5000
3640 rem : GETA$
3650 rem IFA$=""THENNEXT
